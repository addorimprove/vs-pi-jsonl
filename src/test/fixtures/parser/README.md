...
`bom-crlf-v1.jsonl` is deliberately UTF-8 with a BOM and CRLF separators. `empty.jsonl` is intentionally zero bytes. `oversized-record.jsonl` is an otherwise-valid fixture whose middle entry exceeds the small per-record limit used by its test. `subagent-child-transcript.jsonl` is a synthetic version 1 pi-subagents artifact covering prompt de-duplication, nested thinking/tool calls and results, redundant lifecycle markers, stdout/stderr, and truncation. All expectations are asserted in `src/test/unit/parser-fixtures.test.ts`; seeded hostile-input checks are in `parser-property.test.ts`.
